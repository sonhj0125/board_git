...
//					console.log(JSON.stringify(json));
		...
		$("button#btn_upload_excel").click(function() {
			...
			if($("input#upload_excel_file").val() == "") {
	            ...
	            // 또는
	            // let formData = new FormData(document.getElementById("excel_upload_frm")); // 폼 태그에 작성된 모든 데이터 보내기  
	            
				$.ajax({
                    url: "<%=ctxPath%>/emp/uploadExcelFile.action",
                    type: "post",
                    data: formData,
                    ...
                    success: function(json) {
                        ...
                        // ~~~ 확인용 : {"result":1}
                        if(json.result == 1) {
                           alert("엑셀파일 업로드를 성공했습니다.");
                           
                        } else {
                           alert("엑셀파일 업로드를 실패했습니다.");
                        }
                    },
                    error: function(request, status, error){
                    	alert("code: "+request.status+"\n"+"message: "+request.responseText+"\n"+"error: "+error);
