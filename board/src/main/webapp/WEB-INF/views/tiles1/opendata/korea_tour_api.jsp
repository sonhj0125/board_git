...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		}); // end of $(document).on("keyup", "input:text[name='searchword']", function(e){})
		...
		}); // end of $(document).on("click", "button#btn_first", function(){})
		
		
		
		$(document).on("click", "button#btn-prev", function(e){
			
			img_display($("input:text[name='searchword']").val(), $(e.target).val());
			
		}); // end of $(document).on("click", "button#btn-prev", function(){})
		...
		}); // end of $(document).on("click", "button#btn_next", function(){})
		...
		}); // end of $(document).on("click", "button#btn_last", function(){})
	...
	function img_display(searchword, pageNo){
		...
		if(Number(pageNo) >= 1){
			...
			$.ajax({
				...
				success:function(json){
					...
					const totalPageCount = Math.ceil(totalCount/20); // 총 페이지 수(12페이지)
					...
					}); // end of data_arr.forEach
					
					// console.log(img_arr);
					
					let v_html = ``;
					
					img_arr.forEach(function(item, index, array){
		...
		} // end of if(Number(pageNo) >= 1)
		
		
	} // end of function img_display
