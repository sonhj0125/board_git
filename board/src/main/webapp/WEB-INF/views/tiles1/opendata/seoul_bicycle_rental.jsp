    ...
    // 따릉이 마커를 표시할 위치와 내용을 가지고 있는 객체 배열
      ...
      $.ajax({
          ...
          success: function(json) {
              ...
              // [{"lendplace_id":"ST-999","statn_addr1":"????? ??? ???? 280","statn_lnt":0.0,"statn_lat":0.0,"statn_addr2":"????? 8?? ???"}]
              
              $.each(json, function(index, item) {
                  var position = {};
                  
                  position.content = "<div class='mycontent'>" +
                                      "  <div class='title'>" + item.statn_addr1 + "</div>" +
                                      "  <div>" +
                                      "    <a href='https://map.kakao.com/link/map/현위치(약간틀림)," + item.statn_lat + "," + item.statn_lnt + "' style='color:blue;' target='_blank'>큰지도</a> " +
                                      "    <a href='https://map.kakao.com/link/to/" + item.statn_addr1 + "," + item.statn_lat + "' style='color:blue;' target='_blank'>길찾기</a>" +
                                      "  </div>" +
                                      "  <div>" + item.statn_addr2 + "</div>" +
                                      "</div>";
