...
public class CommonAop {
	...
	*/
	...
	public String checkAuthority(ProceedingJoinPoint joinPoint) {
		/* Around Advice 에서는 ProceedingJoinPoint joinPoint 가 포인트컷 되어진 주업무의 메소드이다. */
	    ...
	    if(loginuser == null || loginuser.getGradelevel() < 10) {
	    	...
	    	try {
				...
				/* 
	              	joinPoint.proceed(); 은 Pointcut 에 설정된 주업무 메소드를 실행하라는 것이다.
		...
		/* 
        	return viewPage; 의 뜻은 주업무 메소드인 public String com.spring.app.employees.controller.EmpController.empmanager_*(..) 쪽으로 넘겨준다는 것이다.
          	그러므로 주업무 메소드의 리턴타입이 String 이므로 viewPage 타입도 String 이어야 하는 것이다.
        */
		
	} // end of public String checkAuthority
